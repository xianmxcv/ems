...
const ChartComponent = (props: IProps) => {
  ...
  useEffect(() => {
    const getList = async () => {
      try {
        setData([])
        //setLoading(true)
        const res = await api.getDeviceConsumptionChart(props.queryParams)
        let data: any = []
  ...
  useEffect(() => {
    const getPieList = async () => {
      try {
        setLoading(true)
        const res = await api.getDeviceConsumptionChartSort(props.queryParams)
        let data: any = []
        let flag = false
        res.data.datasets.map((item) => {
          if (item.value < 0) {
    ...
    <div style={{ padding: '8px' }}>
      <Spin spinning={loading}>
        <Row>
          ...
          <Col span={24}>
            {props.chartType === 'line' && <Line {...(lineConfig as any)} loading={loading} />}
