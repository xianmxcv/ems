...
const ComponentTable = (props: IProps) => {
  ...
  useEffect(() => {
    const getTableData = async () => {
      try {
        setLoading(true)
        const res = await api.getItemConsumptionPage({ ...props.queryParams, current, size })
        if (res.data) {
          let arr: any = [
            {
              title: '序号',
        ...
        >
          ...
          <Button
            ...
            onClick={(event) =>
              handleDownFile(
                event,
                `/energy-config/reportItem/export?begin=${props.queryParams.begin}&end=${
