...
import AddGasMeter from './addGasMeter'
...
const GasMeter = () => {
  ...
  const columns: ColumnType<IElectricMeter>[] = [
    ...
    },
    {
      ...
      filters: Object.entries(IGasMeterTypeName).map((item) => ({ text: item[1], value: item[0] })),
    },
    {
      title: '型号',
      dataIndex: 'emModel',
  ...
  return (
    <>
      <Card>
        <Row align="middle">
          ...
          <Col span={18} style={{ textAlign: 'right' }} className={styles['operate-button']}>
            <Space>
              <Input.Search onSearch={handleSearch} placeholder="燃气表名称/编码" allowClear />
      ...
      <Modal
        ...
        destroyOnClose
      >
        <AddGasMeter emInfo={emInfo} onOk={onOk} onCancel={handleCancel} />
      </Modal>
    </>
  )
}
